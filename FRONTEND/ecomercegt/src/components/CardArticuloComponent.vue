<template>
    <div class="col">
        <div class="card h-100">
            <img :src="articulo.imagen" class="card-img-top" alt="imagen producto">
            <div class="card-body">
                <h5 class="card-title">{{ articulo.nombre }}</h5>
                <p class="card-text overflow-ellipsis"><small>{{ articulo.descripcion }}</small></p>
                <p class="card-text">Valor: Q.{{ articulo.precio }}</p>
                <RouterLink :to="{name:'Articulo',params:{id:$props.articulo.codigo}}" class="btn btn-outline-primary btn-sm">Comprar</RouterLink>
            </div>
        </div>
    </div>
</template>
