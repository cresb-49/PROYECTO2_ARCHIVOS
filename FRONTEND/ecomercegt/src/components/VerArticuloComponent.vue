<template>
    <div>
        <div align="center">
            <img  :src="articulo.imagen" alt="Imagen del Articulo">
        </div>
        <h4>{{ articulo.nombre }}</h4>
        <p>{{ articulo.descripcion }}</p>
        <h5> Valor: Q. {{ articulo.precio }}</h5>
        <div class="row justify-content-between">
            <button v-if="$store.state.isAuthenticated && $store.state.role === 'USUARIO'" @click="agregarCarrito" class="btn btn-outline-success col" style="margin: 5px;">Agregar al Carrito</button>
            <button v-if="$store.state.isAuthenticated && $store.state.role === 'USUARIO'" @click="procesarCompra" class="btn btn-outline-warning col" style="margin: 5px;">Procesar Compra</button>
        </div>
    </div>
</template>
...
export default {
    ...
    mounted(){
        this.identificador = this.$route.params.id;
